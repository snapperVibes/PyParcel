...
from pyparcel.common import BOT_ID
...
class EventDetails:
    ...
    cecase_id: int
    # Todo: Cursor typing
    # Although it technically isn't an event detail,
    # passing the db_cursor makes life easier
    db_cursor: Any
...
def query_propertyexternaldata_for_changes_and_write_events(
    ...
    try:
        old = selection[1]
    # If this is the first time the property_propertyid occurs in propertyexternaldata:
    except IndexError:
        if not new_parcel:
            # TODO: Add flag
            print(
                Fore.YELLOW,
                "Error: Parcel {} appeared in public.propertyexternaldata for the "
                "first time even though the parcel ID is flagged as appearing in "
                "public.property before.\n".format(
                    parid
                ),
                Style.RESET_ALL,
                sep="",
...
class ParcelChangedEvent(Event):
    ...
    def __init__(self, d: EventDetails):
        super().__init__(d)
        self.eventdescription = f"Parcel {d.parid}'s {self.brief_description} should " \
                                f"be changed from {d.old} to {d.new} "
        self.active = True
        self.notes = None  # Todo: Add notes
        self.occ_period = None
...
class DifferentTaxCode(ParcelChangedEvent):
    def __init__(self, details):
        ...
        warnings.warn(
            "DifferentTaxCode may be deprecated soon, as TaxCode is no longer an "
            "attribute on the property table",
            DeprecationWarning,
        )
...
def parcel_not_in_wprdc_data(details: EventDetails) -> Event:
    """
    Factory function to be called when a parcel is in our database but not the WPRDC.
